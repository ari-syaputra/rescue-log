        ...
        <div class="flex items-center justify-between border-b border-blue-50 pb-2">
            <div class="flex items-center gap-2">
                ...
                <span class="w-5 h-5 rounded-full bg-red-600 text-white font-bold text-[10px] flex items-center justify-center">
                    {{ isset($sosFeeds) ? count($sosFeeds) : (isset($requests) ? count($requests) : 0) }}
                </span>
            </div>
        </div>

        @php
            $itemsSos = isset($sosFeeds) ? $sosFeeds : (isset($requests) ? $requests : collect());
        @endphp

        <div class="space-y-2.5 max-h-56 overflow-y-auto">
            @forelse($itemsSos as $req)
                <div class="p-3.5 {{ ($req->status ?? '') === 'menunggu_penanganan' ? 'bg-red-50/60 border-red-200/80' : 'bg-blue-50/50 border-blue-100' }} border rounded-xl space-y-2.5">
                        ...
                        <div>
                            ...
                            @else
                                <span class="px-1.5 py-0.5 bg-blue-700 text-white font-black text-[9px] rounded-md uppercase">
                                    {{ str_replace('_', ' ', $req->status ?? 'menunggu') }}
                                </span>
                            @endif
                            ...
                            <p class="text-[11px] text-blue-700/70">{{ $req->lokasi_penjemputan ?? $req->lokasi_detail ?? ($req->posko->nama_posko ?? 'Lokasi tidak spesifik') }}</p>
                        </div>
                        <span class="text-[10px] text-blue-500 font-semibold">
                            {{ isset($req->created_at) ? \Carbon\Carbon::parse($req->created_at)->format('H:i') . ' WIB' : '-' }}
                        </span>
                    </div>
                    ...
                    <a href="{{ Route::has('komando.sos.index') ? route('komando.sos.index') : '#' }}" 
                       class="w-full h-9 inline-flex items-center justify-center px-4 py-2 {{ ($req->status ?? '') === 'menunggu_penanganan' ? 'bg-red-600 hover:bg-red-700 focus:ring-red-200' : 'bg-blue-700 hover:bg-blue-800 focus:ring-blue-200' }} active:bg-blue-900 text-white text-xs font-bold rounded-lg shadow-xs hover:shadow transition-all duration-150 gap-2 cursor-pointer focus:ring-4">
    ...
    <div class="bg-white p-4 rounded-2xl border border-blue-100/80 shadow-xs space-y-3">
        <div class="flex items-center justify-between border-b border-blue-50 pb-2">
            ...
            <span class="w-5 h-5 rounded-full bg-orange-500 text-white font-bold text-[10px] flex items-center justify-center">
                {{ isset($permintaanList) ? count($permintaanList) : (isset($permintaanLogistik) ? count($permintaanLogistik) : 0) }}
            </span>
        </div>

        @php
            $itemsReq = isset($permintaanList) ? $permintaanList : (isset($permintaanLogistik) ? $permintaanLogistik : collect());
        @endphp

        <div class="space-y-2.5 max-h-56 overflow-y-auto">
            @forelse($itemsReq as $item)
                <div class="p-3 bg-blue-50/40 rounded-xl border border-blue-100 space-y-2 text-[11px]">
                    <div class="flex justify-between items-center font-bold">
                        ...
                        <span class="text-amber-700 bg-amber-100/80 px-2 py-0.5 rounded-md border border-amber-200 text-[10px]">
                            {{ str_replace('_', ' ', ucfirst($item->status ?? 'menunggu')) }}
                        </span>
                    </div>

                    <p class="text-blue-700/80 font-medium">
                        Kode: <span class="font-bold text-blue-900">{{ $item->kode_pengajuan ?? '-' }}</span> 
                        @if(($item->beras_kg ?? 0) > 0) | Beras: {{ $item->beras_kg }} Kg @endif
                        @if(($item->air_minum_dus ?? 0) > 0) | Air: {{ $item->air_minum_dus }} Dus @endif
                    </p>
