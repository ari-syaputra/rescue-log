...
@php
    ...
    // 2. Total Otomatis Penerima Manfaat dari Data Pengungsi
    $totalPenerimaManfaat = $pendataan ? $pendataan->total_pengungsi : 0;

    // 3. Status Rekomendasi AI & Tanggal Pembaruan
    $isAiActive = !empty($estimasi);
    $statusText = $isAiActive ? 'Aktif' : 'Manual';
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 md:p-6 flex items-center gap-4">
        <x-sub-posko.pengajuan.stats-card
            ...
            :value="number_format($totalPenerimaManfaat, 0, ',', '.') . ' Jiwa'"
