...
@section('content')
    <div class="w-full space-y-6" x-data="{ isLoading: true }" x-init="setTimeout(() => isLoading = false, 400)">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                ...
                <!-- TAUTAN KE DEDICATED PAGE -->
                <a href="{{ route('lapangan.pengungsi.create') }}"
                ...
                <div class="space-y-6 w-full">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden w-full p-4 sm:p-0">
                        ...
                        <div class="block sm:hidden space-y-3">
                            @forelse ($riwayat_pendataan as $riwayat)
                                <div class="p-4 bg-gray-50/70 rounded-xl border border-gray-200/80 flex flex-col gap-2.5">
                                    <div class="flex items-center justify-between border-b border-gray-200/60 pb-2">
                                        ...
                                        <span class="px-2.5 py-0.5 text-xs font-bold bg-blue-100 text-blue-700 rounded-full">
                                            {{ $riwayat->total_pengungsi ?? 0 }} Jiwa
                                        </span>
                                    </div>
                                    <div class="grid grid-cols-2 gap-2 text-xs pt-1">
                                        <div>
                                            <span class="text-gray-400 block text-[10px] uppercase font-semibold flex items-center gap-1">
                                                <x-heroicon-s-building-office-2 class="w-3.5 h-3.5 text-emerald-500" /> Tipe Fasilitas
                                            </span>
                                            <span class="font-medium text-gray-800">{{ $riwayat->tipe_tempat ?? '-' }}</span>
                                        </div>
                                        <div class="text-right">
                                            <span class="text-gray-400 block text-[10px] uppercase font-semibold flex items-center justify-end gap-1">
                                                <x-heroicon-s-sun class="w-3.5 h-3.5 text-amber-500" /> Cuaca
                                            </span>
                                            <span class="font-medium text-gray-700">
                                                {{ $riwayat->cuaca ?? '-' }}
                                                @if (isset($riwayat->suhu_celcius))
                                                    <span class="text-[11px] text-gray-400 font-normal">({{ $riwayat->suhu_celcius }}°C)</span>
                                                @endif
                                            </span>
                                        </div>
                                    </div>
                            ...
                            @empty
                                <div class="py-8 text-center text-gray-400 italic text-xs bg-gray-50 rounded-xl border border-dashed border-gray-200">
                        ...
                        <div class="hidden sm:block overflow-x-auto">
                            <table class="w-full text-left text-sm">
                                <thead class="text-gray-500 bg-white border-b border-gray-200">
                                    <tr>
                                        <th class="px-6 py-3 font-semibold"><span class="flex items-center gap-1.5"><x-heroicon-s-calendar class="w-4 h-4 text-gray-400" /> Tanggal Update</span></th>
                                        <th class="px-6 py-3 font-semibold"><span class="flex items-center gap-1.5"><x-heroicon-s-users class="w-4 h-4 text-blue-500" /> Total Pengungsi</span></th>
                                        <th class="px-6 py-3 font-semibold"><span class="flex items-center gap-1.5"><x-heroicon-s-building-office-2 class="w-4 h-4 text-emerald-500" /> Tipe Fasilitas</span></th>
                                        <th class="px-6 py-3 font-semibold text-right"><span class="inline-flex items-center justify-end gap-1.5"><x-heroicon-s-sun class="w-4 h-4 text-amber-500" /> Cuaca Tercatat</span></th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @forelse ($riwayat_pendataan as $riwayat)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-6 py-4 font-medium text-gray-900">{{ $riwayat->created_at ? $riwayat->created_at->format('d M Y, H:i') : '-' }}</td>
                                            <td class="px-6 py-4 text-blue-600 font-bold">{{ $riwayat->total_pengungsi ?? 0 }} Jiwa</td>
                                            <td class="px-6 py-4 text-gray-700">{{ $riwayat->tipe_tempat ?? '-' }}</td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-6 py-8 text-center text-gray-400 italic">Belum ada riwayat perubahan data.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.addEventListener('online', syncOfflinePendataanQueue);
            ...
            async function syncOfflinePendataanQueue() {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        text: 'Data pendataan pengungsi offline berhasil dikirim ke server.',
                        icon: 'success',
